...
def pad_dt(dt):#
    padding_shape = dt[0][0].shape
    for i in tqdm(range(dt.shape[0]),desc=f'Padding Start Token of shap {padding_shape}'):
        dt[i] = np.concatenate([np.zeros((1,66)),dt[i]])
    return dt
...
dataset = 
ckpt = tf.train.Checkpoint(step=tf.Variable(1), optimizer=optimizer, net=tvae, iterator=iter(dataset))
...
tvae.compile( optimizer=optimizer, run_eagerly=True )
tvae.fit( test_sample, epochs=10, batch_size=8 )
...
ori = test_sample[0]
encoded = tvae.encoder(test_sample[0][np.newaxis,:,:])[0]
decoded = tvae.decoder(test_sample[0][np.newaxis,:,:][:,:-1,:],tvae.encoder(test_sample[0][np.newaxis,:,:])[0])
...
positional_encoding(1, 20)[np.newaxis,:,:]


# In[ ]:


start_token = positional_encoding(1, 66)[np.newaxis,:,:]#np.repeat(0,66)[np.newaxis,np.newaxis,:]
def get_latent_vector(sentence):
  if len(sentence.shape)==2:
    sentence = sentence[np.newaxis,:,:]
  encoded = tvae.encoder(sentence)[0]
  latent_vector = tvae.decoder(x=start_token,context=encoded)
  #latent_vector = tvae.dense(decoded)
  return tf.squeeze(latent_vector,axis=(0,1))
...
eg = dt[20][np.newaxis,:,:]
dec = dt[20][np.newaxis,:-1,:]
out = dt[20][np.newaxis,1:,:]
...
encoded = tvae.encoder(eg)[0]
a = tvae.decoder(x=dec,context=encoded).numpy()
...
output = []
for i in range(3): 
    a = get_latent_vector(dt[i]).numpy()
    output.append(a)
    print(a)
...
import pandas as pd
pd.DataFrame(a.squeeze())


# In[ ]:


# need to add something to save the model
